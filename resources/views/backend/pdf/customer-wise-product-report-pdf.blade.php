...
<html lang="en">
  ...
  <!-- Theme style -->
    ...
    <title>Individual Customer All Invoice Repoet</title>
  </head>
  <body>
        ...
        <div style="font-size: 18px;margin-top: 7px;font-weight: bold;text-align: center;"><u ><i>Individual Customer All Invoice Repoet</i></u></div>
           <br>
            ...
            <div class="card-body">
                ...
                <table width="100%" class="table table-bordered table-sm" border="1" >
                  <tbody>
                    ...
                    <tr>
                     ...
                     <tr>
                      <td width="17%">{{$payment['0']['customer']['name']}}</td>
                      <td width="17%">{{$payment['0']['customer']['shop_name']}}</td>
                      <td width="16%" style="text-align: center;">{{$payment['0']['customer']['mobile']}}</td>
                      <td width="18%">{{$payment['0']['customer']['email']}}</td>
                      <td width="33%">{{$payment['0']['customer']['address']}}</td>
                    </tr>
                  </tbody>
                </table>
                <br>
                 ...
                 <table width="100%" border="1" class="table table-bordered table-sm" style="margin-bottom: 15px;padding-bottom: 0">
                  ...
                  <tbody>
                     @php
                   $total_sum = '0';
                    // $invoicedetails = App\Model\InvoiceDetail:: where('invoice_id',$payment->customer_id)->get();
                   @endphp
                   @foreach($invoicedetails as $key => $details)
                    <tr>
                     <td style="text-align: center;">{{$key+1}}</td>
                     <td style="text-align: center;">{{$details->invoice_id}}</td>
                     <td style="text-align: center;">{{date('d-m-Y',strtotime($details['invoice']['invoice_date']))}}</td>
              ...
              {{--   <table width="100%" border="1" class="table table-bordered table-sm" style="margin-bottom: 15px;padding-bottom: 0">
                  ...
                  <tbody>
                    @php
                    $total_pay = '0';
                   $payment = App\Model\Payment::where('customer_id',$details->customer_id)->get();
                   @endphp
